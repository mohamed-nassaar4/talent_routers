...
exports.todoCreate = async (req, res) => {
  try {
    ...
    let todoExist = await db.all(`select * from TODO where user_id='${parseInt(user_id)}' and status =1 order by created_dttm desc limit 10`);
    ...
    if (todoExist.length == 0) {
      ...
      let firstEntry = moment(todoExist[todoExist.length - 1].created_dttm, 'YYYY-MM-DD HH:mm:ss');
      ...
      if (todoExist.length == 10 || differenceInMinutes > 15) {
        console.log('ss')
        ...
        if (todoExist.length == 10 && delayMinutes >= 3) {
          todoExist.forEach(async (data) => {
            await db.run(`UPDATE  TODO  SET status = 0 where todo_id=?`, [parseInt(data.todo_id)])
          })
          return res.send({ msg: 'Limit Exceed' })
        }
        return res.send({ msg: 'Limit Exceeds' })
      }
